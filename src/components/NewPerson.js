...
const NewPerson = (props) => {
    ...
    const createNewPerson = e =>{
        e.preventDefault();
        //send via axios
        clienteAxios.post('/people', person)
            .then(res => {
                console.log(res);
                props.reCheck(true);
                //redirect
    ...
    return (
        <Fragment>
            <h1 className="my-5">Add a new person to database</h1>
            ...
            <form onSubmit={createNewPerson} className="bg-white p-5 bordered">
        ...
        <input type="submit" className="btn btn-primary mt-3 w-100 p-3 text-uppercase font-weight-bold" value="Create Person" />
</form>
            </div>
        </Fragment>
    );
